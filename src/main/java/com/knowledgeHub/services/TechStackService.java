...
public class TechStackService {
	...
	public ResponseEntity<?> deleteByName(String name) {
		...
		if (techStackOpt.isEmpty()) {
			return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Tech stack not found");
		}
	...
	public ResponseEntity<?> saveAll(List<TechStackDTO> dtoList) {
		...
		List<String> existing = repository.findAll().stream().map(TechStack::getTechstackName).toList();
		...
		List<TechStack> newList = dtoList.stream()
				...
				.filter(dto -> !existing.contains(dto.getTechstackName())).map(dto -> {
		...
		if (newList.isEmpty()) {
			return ResponseEntity.badRequest().body("All tech stacks already exist");
		}
	...
	public void updateTechStack(String oldName, String newName) {

		// ✅ Find existing tech
		TechStack tech = repository.findByTechstackNameIgnoreCase(oldName)
				.orElseThrow(() -> new RuntimeException("Tech stack not found"));

		// ✅ Check duplicate
		if (repository.existsByTechstackName(newName)) {
			throw new ResponseStatusException(HttpStatus.CONFLICT, "Tech stack already exists");
		}

		// ✅ Update name
		tech.setTechstackName(newName);
